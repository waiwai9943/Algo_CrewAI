...
backtest_usatech.py
-------------------
Run VWAP+RSI mean-reversion backtest directly on QuantDataManager CSV export for US Tech Index (NQ equivalent).

Data: C:\QuantDataManager125\export\2026.6.7USATECHIDXUSD-M5-No Session.csv
      (US Tech Index USD, 5-minute bars)
"""
...
# Parse datetime. Format: Date '20110919', Time '13:30:00'
...
# Use only last 2 years for reasonable backtest scope
...
# ─────────────────────────────────────────────────────────────────
# Backtest Engine (US Tech / NQ equivalent)
# ─────────────────────────────────────────────────────────────────

# For NQ futures, 1 full point = $20.
POINT_VALUE = 20.0
COMMISSION_RT = 4.10
SLIPPAGE_POINTS = 0.25  # 1 tick slippage
...
print("\nRunning backtests on US Tech Index (NQ) QDM data...\n")

# Points = NQ ticks / 4 (since 4 ticks per point)
# Base TP 80 ticks = 20 points. SL 40 ticks = 10 points.
variants = [
    ("Base (z=±1.5, RSI 35/65, TP 20pts SL 10pts)",   -1.5, 35.0, 1.5, 65.0, 20.0, 10.0),
    ("Tight (z=±2.0, RSI 30/70, TP 20pts SL 10pts)",  -2.0, 30.0, 2.0, 70.0, 20.0, 10.0),
    ("Loose (z=±1.0, RSI 40/60, TP 25pts SL 12.5pts)", -1.0, 40.0, 1.0, 60.0, 25.0, 12.5),
]
...
with open("backtest_results_usatech.json", "w") as f:
    json.dump(all_results, f, default=str, indent=2)
print("Saved to backtest_results_usatech.json")
